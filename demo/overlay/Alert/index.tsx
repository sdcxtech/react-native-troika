...
function ModalBase({ overlayId, passThroughTouches }: OverlayProps) {
	...
	return (
		<View style={styles.container} pointerEvents={passThroughTouches ? 'box-none' : 'auto'}>
			<Animated.View
				style={[styles.mask, StyleSheet.absoluteFill, { opacity: opacity.current }]}
			/>
...
function showInternal() {
	registerIfNeeded();
	key += 1;
	Overlay.show('__overlay_alert__', { passThroughTouches: false, overlayId: key });
	return key;
}

function hideInternal(overlayId: number) {
	propsMap.delete(overlayId);
	Overlay.hide('__overlay_alert__', overlayId);
}
	...
	alert(title: string, message: string, buttons: AlertButton[]) {
		...
		const params = {
			...
			buttons: buttons.map(button => ({
				...button,
				onPress: () => {
					hideInternal(overlayId);
					button.onPress();
				},
			})),
		};
		propsMap.set(overlayId, params);
	},
};
