...
import com.reactnative.imagecrop.ucrop.callback.OverlayViewChangeListener;
...
//        }
    ...
    public void setupDetectedObjectBounds(float imageWidth, float imageHeight, float top, float left, float width, float height) {
        postDelayed(new Runnable() {
            @Override
            public void run() {
                int halfDiff = (mThisHeight - (int) (mThisWidth / mTargetAspectRatio)) / 2;

                float mLeft = getPaddingLeft() + mThisWidth * (left / imageWidth);
                float mTop = getPaddingTop() + halfDiff + (mThisWidth / mTargetAspectRatio) * top / imageHeight;
                float mRight = mLeft + mThisWidth * width / imageWidth;
                float mBottom = mTop + mThisWidth / mTargetAspectRatio * height / imageHeight;
                mCropViewRect.set(mLeft, mTop, mRight, mBottom);
                if (mCallback != null) {
                    mCallback.onCropRectUpdated(mCropViewRect);
                }
                updateGridPoints();
