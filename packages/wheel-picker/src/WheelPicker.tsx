...
const WheelPickerNative =
    requireNativeComponent<WheelPickerNativeProps>('WheelPicker');
...
function WheelPicker<T>({
    ...
    const itemHeight =
        _itemStyle.height && typeof _itemStyle.height === 'number'
            ? _itemStyle.height
            : lineHeight;
    const fontSize = _itemStyle.fontSize ?? 14;
    const colorCenter =
        _itemStyle?.color && typeof _itemStyle.color === 'string'
            ? _itemStyle.color
            : undefined;
    const colorOut =
        _style?.color && typeof _style.color === 'string'
            ? _style.color
            : colorCenter;
